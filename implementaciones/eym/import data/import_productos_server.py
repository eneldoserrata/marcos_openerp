...
conn = psycopg2.connect(dbname='xxxxx', user='xxxx', password='xxxx', host="xxxxx")
...
for row in rows:
    ...
    if row[40]:
        try:
            i = open('fotos/{}'.format(row[40]), 'rb').read()
    ...
    produc = {
        ...
        #"PRO_FOTO": img,
        "PRO_MODDESCRIPCION": row[34],
        "PRO_UTI1": row[35],
        "PRO_UTI2": row[36],
        "PRO_UTI3": row[37],
        "PRO_ATE1": row[38],
        "PRO_ATE2": row[39],
        "IMAGEN":  row[40] or None,
        "DESCRIPCION": None,#row[41],
        "WEB": row[42],
        "PRO_PRECIO5": row[43],
        "PRO_PRECIO6": row[44],
        "PRO_PRECIO7": row[45],
    ...
    product_supplier_taxes_rel_data = (id, 8)
    ...
    product_product_data = (                        id,              id,           True,   'real_time', img_big,                      True,              img_med,                       img_med,                         1,          )
