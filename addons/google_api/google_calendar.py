    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for id in ids:            
            ...
            if any(k in vals for k in ['name', 'description', 'location', 'date', 'date_deadline']):                                                        
                google_sequence = self.read(cr, uid, id, ['google_sequence'])['google_sequence']
                vals['google_sequence'] = google_sequence + 1                
                super(crm_meeting, self).write(cr, uid, id, vals, context=context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for id in geventids:
            if id['google_event_id'] and id['user_id']:
                ...
                instance_pool = self.pool.get('google.api.calendar')                
                instance = instance_pool.search(cr, uid, [('user_id', '=', id['user_id'][0])])                    
                if instance:
                    instance = instance_pool.browse(cr, uid, instance[0])
                    try:                                                        
                        storage = file.Storage(instance.account_id.credential_path)        
                        ...
                        http = credentials.authorize(http)                                    
                        service = discovery.build('calendar', 'v3', http=http)
                        service.events().delete(calendarId=instance.calendar_id, eventId=id['google_event_id']).execute()                        
                    except:
