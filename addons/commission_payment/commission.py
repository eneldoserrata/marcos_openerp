...
class commission_payment(osv.Model):
    ...
    def prepare(self, cr, user, ids, context={}):
        ...
        for commission in commissions:
            ...
            if not voucher_ids:
                ...
                voucher_ids = vouchers.search(cr, user, [('state', '=', 'posted'), (
                    ...
                    for payment_brw in voucher_brw.line_cr_ids:
                        ...
                        if pay_line_vendor in user_ids:
                            ...
                            if payment_brw.invoice_id and not payment_brw.paid_comm:
                                ...
                                wh_lines = inv_brw.wh_iva_id and inv_brw.wh_iva_id.wh_lines and [
                                    line.wh_iva_rate for line in inv_brw.wh_iva_id.wh_lines if inv_brw.id == line.invoice_id.id] or []
                                ...
                                mun_ids = mun_obj.search(cr, user, [('invoice_id', '=', inv_brw.id)]) or mun_obj.search(
                                    cr, user, [('invoice_id.partner_id', '=', inv_brw.partner_id.id)])

                                perc_ret_im = mun_ids and \
                                    [mun_brw.wh_loc_rate for mun_brw in mun_obj.browse(cr, user, mun_ids)] and \
                                    max([mun_brw.wh_loc_rate for mun_brw in mun_obj.browse(
                                         cr, user, mun_ids)]) or 0.0
                                    ...
                                    for inv_lin in inv_brw.invoice_line:
                                        ...
                                        if inv_lin.product_id:
                                            ...
                                            for price_id in price_ids:
                                                ...
                                                prod_prices_brw = prod_prices.browse(
                                                    cr, user, price_id, context=None)
                                            ...
                                            if no_price == False:
                                                ...
                                                pay_date = datetime.strptime(
                                                    voucher_brw.date, '%Y-%m-%d')
                                                inv_date = datetime.strptime(
                                                    inv_brw.date_invoice, '%Y-%m-%d')
                                                emission_days = (
        ...
        for commission in commissions:
            ...
            for comm_line in commission.comm_line_ids:
                ...
                if len(criba[vendor_id][voucher_id][1][invoice_id]) == 0:
                    criba[vendor_id][voucher_id][1][invoice_id] = [
                        [], comm_line.pay_inv, comm_line.perc_ret_iva, comm_line.perc_ret_islr, comm_line.perc_ret_im]
    ...
    def pre_process(self, cr, user, ids, context={}):
        ...
        for commission in commissions:
            ...
            if not commission.noprice_ids:
                ...
                raise osv.except_osv(_('Atencion !'), _(
                    'Debe primero solucionar el asunto de los Productos sin Listas de Precio \npara las fechas especificadas antes de continuar'))
        return True
    ...
    def unlink(self, cr, user, ids, context={}):
        ...
        for commission in commissions:
            ...
            uninvoiced_pays.unlink(cr, user, [
                                   line.id for line in commission.uninvoiced_ids])
            ...
            comm_voucher_ids.unlink(cr, user, [
                                    line.id for line in commission.comm_voucher_ids])
            # * Desvinculando los vouchers afectados
            comm_invoice_ids.unlink(cr, user, [
                                    line.id for line in commission.comm_invoice_ids])
            # * Desvinculando las facturas con problemas de retenciones
            comm_retention_ids.unlink(cr, user, [
                                      line.id for line in commission.comm_retention_ids])
            ###
